  ...
  ko: [
    {
      ...
      label: "위탁 운영 노하우 보기",
      link: "/service/operation" as Route,
    },
    {
      id: 2,
      icon: MarketingGrowthIcon,
      label: "마케팅 노하우 보기",
      link: "/service/marketing" as Route,
    },
    {
      id: 3,
      icon: ConstructionBuildIcon,
      label: "시공 · 건축 노하우 보기",
      link: "/service/construction" as Route,
    },
  ],
  en: [
    {
      id: 1,
      icon: OperationsManagementIcon,
      label: "View Our Asset Management Know-How",
      link: "/service/operation" as Route,
    },
    {
      id: 2,
      icon: MarketingGrowthIcon,
      label: "View Our Marketing Expertise",
      link: "/service/marketing" as Route,
    },
    {
      id: 3,
      icon: ConstructionBuildIcon,
      label: "View Our Construction & Development Experience",
      link: "/service/construction" as Route,
    },
  ],
} as const;
...
export default function GrowthCta() {
  ...
  const buttons = ctaButtons[locale];
    ...
    <section className="relative flex min-h-100 items-center justify-center overflow-hidden md:min-h-112.5 lg:min-h-130.5">
      ...
      <div className="relative z-10 flex max-w-7xl flex-col items-center px-2.5 py-12 text-center md:px-9">
        <h2 className="font-chosunilbo text-white text-xl leading-tight md:text-2xl lg:text-[43px]">
          {isEn
            ? "Maximize Revenue and Asset Value Together"
        ...
        <div className="pt-4 pb-6 font-light text-sm text-white md:pt-6.25 md:pb-11.5 md:text-base">
          {isEn ? (
            <>
              <p>Through aggressive revenue strategies that maximize occupancy</p>
              <p>and rigorous operational management, performance improvement is inevitable.</p>
              <p className="mt-3 font-semibold">Explore Our Expertise</p>
            </>
          ) : (
            <>
        ...
        <div className="flex w-full flex-col items-center gap-3 lg:flex-row lg:justify-center">
          {buttons.map((item) => (
            ...
            >
              <Image src={item.icon} alt="" className="h-8 w-8 shrink-0 md:h-10 md:w-10" />
              <span className="font-normal text-base text-white md:text-lg lg:text-xl">
                {item.label}
              </span>
            </Link>
          ))}
